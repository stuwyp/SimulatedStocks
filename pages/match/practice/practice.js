...
Page({
                    ...
                    wx.request({
                        ...
                        success(res) {
                            ...
                            if (res.data.value === 1) {
                                ...
                                /*以下为获取比赛时间期间的各类型订单 */
                                let match_data = that.data.match_data
                                that.getUserOrder(token, 1, String(match_data.start_time), String(match_data.end_time), 1)
                                that.getUserOrder(token, 1, String(match_data.start_time), String(match_data.end_time), 2)
                                that.getUserOrder(token, 2, String(match_data.start_time), String(match_data.end_time), 1)
                                that.getUserOrder(token, 2, String(match_data.start_time), String(match_data.end_time), 2)
                                wx.hideLoading()
                                wx.showToast({
                                    title: '购买成功',
                                })
    ...
    getWarehouse: async function (token, matchid) {
        try {
            let res = await wxRequest({
    ...
    fetchData: function (options) {
        ...
        /*以下为获取比赛时间期间的各类型订单 */
        this.getUserOrder(token, 1, String(match_data.start_time), String(match_data.end_time), 1)
        this.getUserOrder(token, 1, String(match_data.start_time), String(match_data.end_time), 2)
        this.getUserOrder(token, 2, String(match_data.start_time), String(match_data.end_time), 1)
        this.getUserOrder(token, 2, String(match_data.start_time), String(match_data.end_time), 2)
